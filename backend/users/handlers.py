...
from security.globals import salt, security_scheme
...
async def get_all_users(credentials: HTTPBasicCredentials = Depends(security_scheme)):
    return await User_Pydantic.from_queryset(Users.all())


@router.post("/")
async def create_user(user: UserIn_Pydantic, credentials: HTTPBasicCredentials = Depends(security_scheme)):
...
async def get_user(id: int, credentials: HTTPBasicCredentials = Depends(security_scheme)):
...
async def update_user(id: int, user: UserIn_Pydantic, credentials: HTTPBasicCredentials = Depends(security_scheme)):
    await Users.filter(id=id).update(**user.dict(exclude_unset=True))
    return await User_Pydantic.from_queryset_single(Users.get(id=id))


@router.delete("/{id}", responses={404: {"model": HTTPNotFoundError}})
async def delete_user(id: int, credentials: HTTPBasicCredentials = Depends(security_scheme)):
